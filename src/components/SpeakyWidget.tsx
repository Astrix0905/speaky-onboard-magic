  ...
  return (
    <div className={`pointer-events-none fixed z-50 flex flex-col gap-3 ${posClasses}`}>
      {open && (
        <div
          key="panel"
          className="speaky-scale-in pointer-events-auto w-[340px] overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl"
        >
          {/* Dark header */}
          ...
          <div className="px-4 py-4">
            {stage === "form" && (
              <div key="form" className="speaky-fade-up space-y-3">
                <input
                  value={name}
            ...
            {stage === "loading" && (
              <div
                key="loading"
                className="speaky-fade-up flex items-center justify-center gap-2 py-6 text-sm text-muted-foreground"
              >
            ...
            {stage === "playing" && (
              <div key="playing" className="speaky-fade-up space-y-3">
                {/* Sound wave */}
      ...
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="pointer-events-auto flex h-12 w-12 items-center justify-center rounded-full bg-brand-navy text-white shadow-xl ring-4 ring-brand-navy/10 transition hover:scale-105 hover:brightness-110 active:scale-95"
